...
function processAction(p) {
  ...
  try {
    if (p.action === "addPost") {
      ss.getSheetByName(S.POSTS).appendRow([new Date(), u, p.author, p.tag, p.title, p.content]); msg = "✅ 發布成功";
    } else if (p.action === "addReply") {
      ss.getSheetByName(S.REPLIES).appendRow([new Date(), u, p.author, p.postTitle, p.replyContent]); msg = "✅ 回覆成功";
...
function loadConversations(email) {
  if (!email) return [];
  return readSheet(S.CONVERSATIONS, 6).filter(r => String(r[1]) === email).map(r => ({
    timestamp: String(r[0]), userEmail: String(r[1]), convId: String(r[2]), role: String(r[3]), message: String(r[4]), personaKey: String(r[5])
  }));
}
